...
- feat: 設定が折りたためるセクションに整理されました
  - 全ての見出しが折りたためるようになり、既定では折りたたまれた状態で開くので、最初に見えるのは設定項目の一覧と変換ボタンです
  - パラメータ関連の設定を1つの見出しに、VRCコンポーネント関連をもう1つの見出しにまとめ、以前の変換方式のために残している設定は末尾の`レガシー`に移しました
  - Step表示を廃止し、各設定の名前を内部の仕組みではなく対象そのものの名前にしました
...
- fix: エモートの後に一定時間アバターが自転車ポーズになり、次のエモートが始まらない問題を修正しました（`ウエイト0ステート`）
  - エモートのレイヤーは、エモートが終わった後の後始末を1つのステートに置きます（標準Actionのブレンドアウト、エモート追加ツールがクリップ1周ぶん待つ後始末）。その時点でActionのレイヤーウエイトは既に落ちているため、VRChatではどちらも見えていませんでした。まとめた後は1つのレイヤーが1つのステートしか再生せず、隠すためのウエイトも無いので、そのまま再生されていました。後始末のクリップは何もアニメーションしないため、出てくるのはUnity Humanoidの既定ポーズです
