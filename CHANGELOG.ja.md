...
- feat: 独自のlocomotionを持つアバターでは、ChilloutVR自身のlocomotionレイヤーをそれで置き換えるようになりました（`Locomotionレイヤーを変換`）。VRChatの`proxy_*`プレースホルダークリップはChilloutVR自身のものに差し替えられ、ChilloutVRの飛行・水泳・エモートの各ステートはアバター自身のステートマシンへ繋ぎ直されるのでそのまま機能します。プレースホルダーしか持たないBaseレイヤーや、1層目にデフォルトステートが無いBaseレイヤーは、警告を出してChilloutVRのlocomotionに任せます
- feat: 着地でChilloutVR自身の着地アニメーションが本来のタイミングで再生されるようになりました（`着地アニメーションを再生する`、既定でON）。OFFだと変換後の着地はポーズ1枚固定になり、体が急に沈みます
- feat: locomotionを担うレイヤーのTracking Controlは変換しないようになりました（`locomotionレイヤーのTracking Controlを変換する`、既定でOFF）。VRChatの着地ステートにはこれが入っていることが多く、変換するとChilloutVRでフルボディトラッキングががたつきます
- feat: Actionレイヤーがlocomotionレイヤーへ畳み込まれ、ChilloutVR自身のエモートメニューで駆動されるようになりました（`Actionレイヤーを変換`）。2層目以降も可能な限り畳み込まれ、代わりを務められないもの（マスク付き、デフォルトweightが0、加算合成、条件付きの入口が無い）は警告を出して飛ばします。locomotionレイヤーを置き換えるかどうかとは無関係に動作します。`VRCEmote`を読むアバターには互換レイヤーが生成され、ChilloutVR自身のエモートパラメータを橋渡しするので、クイックメニューのエモートとキャンセルがそのまま効きます
- feat: Sittingレイヤーも同じように畳み込まれるようになりました（`Sittingレイヤーを変換`）。対象は独自の着席アニメーションを持つアバターのみで、標準のSittingレイヤーはChilloutVR自身の着席ポーズに任せます。2層目以降は畳み込まれません
- feat: `TrackingType`をゲームから供給するようになりました。ChilloutVRはフルボディトラッキングが有効かどうかしか把握していないため、3（頭と手）と6（フルボディ）のみを生成します。腰のみ・足のみはフルボディと区別できず、genericの1に相当するものはありません
- feat: Playable Layersが既定ですべて変換されるようになりました。各変換は、ChilloutVRが既にやっていることの代わりを務められるかを自分で判断し、務まらない場合はChilloutVR自身のレイヤーをそのまま残すので、手動でONにする必要が無くなりました。既に`VRC3CVR Avatar`コンポーネントが付いているアバターは、保存済みの設定をそのまま保ちます
- feat: locomotionのオプションから`非推奨`の表記が外れました。Additiveのオプションにあった自転車ポーズの注意書きも、下記の加算合成の修正で原因が無くなったため外しました
- fix: Additiveのレイヤーが加算合成されるようになりました。Additive playableはコントローラの設定ではなくプラットフォームの規則によって加算になるため、コントローラ側は通常Overrideのまま置かれています。これをそのまま持ち込んでいたので、合成後のポーズに足すのではなく丸ごと置き換えていました。また1層目のアバターマスクも適用しないようになりました。VRChatがこれを無視するため、アバターは「効かないもの」として作られているからです
- fix: `VelocityX` / `VelocityZ` がアバターローカルになりました。ChilloutVRがanimatorに渡すのはワールド座標の速度なので、VRChatのアバターローカルな値を前提に作られたブレンドツリーはアバターの向きによって違うモーションを再生していました。これらを読む変換済みレイヤーは、生成されたアバターローカルの値を参照するようになります。同期の枠は消費しません
- fix: intドロップダウンの選択肢と、それが書き込む値が一致するようになりました。ChilloutVRのドロップダウンはリスト内のインデックスで値を指定するため、選択肢の値が0から始まらないアバターでは表示と異なる値が書き込まれていました
- fix: エモート中に手のジェスチャーが止まるようになりました。ChilloutVRは`LeftHand` / `RightHand`という名前のレイヤーを探して手をミュートしますが、変換後のGestureレイヤーはその名前ではなかったため、エモート中も指が動き続けていました
