...
- feat: Actionレイヤーがlocomotionレイヤーへ畳み込まれ、ChilloutVR自身のエモートメニューから駆動されるようになりました（`Actionレイヤーを変換`）
  ...
  - 2層目以降も畳み込まれますが、畳み込みでは代替できない手段でエモートを抑えているレイヤー（アバターマスク、デフォルトweightが0、加算合成、1つ目のステートから出る条件付き遷移が無い、AnyStateからそのステートに戻る、独自のサブステートマシンを持つ）は対象外です。該当するレイヤーはエモートが丸ごと失われるため、理由を挙げた警告を出して飛ばします
- feat: Sittingレイヤーも同じように畳み込まれるようになりました（`Sittingレイヤーを変換`）
...
- fix: エモートの後に一定時間アバターが自転車ポーズになり、次のエモートが始まらない問題を修正しました（`ウエイト0ステート`）
  - エモートのレイヤーは、エモートが終わった後の後始末を1つのステートに置きます（標準Actionのブレンドアウト、エモート追加ツールがクリップ1周ぶん待つ後始末）。その時点でActionのレイヤーウエイトは既に落ちているため、VRChatではどちらも見えていませんでした。畳み込み後は1つのレイヤーが1つのステートしか再生せず、隠すためのウエイトも無いので、そのまま再生されていました。後始末のクリップは何もアニメーションしないため、出てくるのはUnity Humanoidの既定ポーズです
  - これらのステートは到達したその場で抜けるようになりました。ステート自体は通るので、そこに載っているものは従来どおり発火します。VRChatと同じタイミングを保ちたい場合は`変更しない`を選べますが、その間の棒立ちは見えます。すぐ抜ける場合は次のエモートがVRChatより早く始まります。1つのレイヤーでは両立できません
  - ウェイトのフェードが残っていた分は、出ていく遷移のブレンドに引き継がれます（ウェイトのクロスフェードと同じもの）。標準Actionは0.5秒のフェードを指示しつつクリップの1/5の時点で抜けるため、エモート後のlocomotionへの復帰は、ハードカットではなく残りの時間でブレンドされるようになりました
  - どのステートが見えていなかったかを機械の形から判定できないレイヤーには手を加えません
