...
public class VRC3CVRConvertConfigDrawer : PropertyDrawer
{
    class T
    {
        ...
        public static istring FeedGameStateParametersDescription => new istring("Feeds these VRChat built-ins from the game via CVR Parameter Stream on the wearer's client, and keeps them synced so remote viewers see the values too (sync cost: VRMode/Upright 32 bits each, MuteSelf joins the 8-bit bool packing; only when the avatar uses them). When the avatar's own locomotion replaces ChilloutVR's, Upright is derived on every client instead of streamed, which moves its 32 bits to a sensor parameter and adds 32 more for the VRMode the derivation reads.", "これらのVRChat組み込みパラメータをCVR Parameter Streamで装着者クライアントから供給し、同期対象にすることでリモートにも値を届けます（同期コスト: VRMode/Upright 各32bit、MuteSelfはBoolの8bitパックに相乗り。アバターが使用している場合のみ）。アバター自身のlocomotionがChilloutVRのものを置き換える場合、Uprightは配信ではなく各クライアントで導出されます。その32bitはセンサー用パラメータに移り、導出が読むVRMode分の32bitが追加で必要になります。");
