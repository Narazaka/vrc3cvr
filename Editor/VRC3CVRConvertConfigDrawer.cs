...
public class VRC3CVRConvertConfigDrawer : PropertyDrawer
{
    class T
    {
        ...
        public static istring ConvertLocomotionTrackingControlDescription => new istring("Applies to every layer merged into the locomotion layer. Tracking Control, common in VRChat's landing states, causes full-body tracking jitter in ChilloutVR when converted.", "locomotionレイヤーにまとめられるレイヤーすべてが対象です。VRChatの着地ステートにありがちなTracking Controlを変換するとCVRでFBTががたつく原因になります。");
        ...
        public static istring ConvertActionAnimatorDescription => new istring("Moves the emotes into the locomotion layer and plays them from ChilloutVR's own emote menu. A layer that cannot be handled is left out, with a warning naming the reason.", "エモートをlocomotionレイヤーにまとめ、ChilloutVR自身のエモートメニューで再生します。対応できないレイヤーは、理由を挙げた警告を出して対象外になります。");
        ...
        public static istring ConvertFXAnimator => new istring("Convert FX Animator (expressions, gimmicks)", "FXレイヤーを変換 (表情、ギミック)");
        public static istring ConvertFXAnimatorDescription => new istring("Expressions, outfit toggles, menu gimmicks -- nearly everything an avatar does that is not how it stands or moves is here. Leave it on unless you have a reason not to.", "表情・衣装の切り替え・メニューのギミックなど、姿勢や移動以外のアバターの機能はほとんどこのレイヤーにあります。理由が無ければONのままにしてください。");
